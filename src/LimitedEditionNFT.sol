...
contract LimitedEditionNFT is Ownable2Step, ERC721, ERC2981 {
  ...
  modifier validateMint(address to, uint256 price) {
    ...
    if (remainingSupply < 1) revert MaxSupplyReached();
  ...
  constructor(
    ...
    bytes32 _merkleRoot
  ) Ownable(msg.sender) ERC721('LimitedEditionNFT', 'LENFT') {
  ...
  function mint(address to, bytes32[] calldata merkleProof) external payable validateMint(to, discountPrice) {
    if (merkleProof.length == 0) revert InvalidProofParams();
    ...
    if (discountBitmap[index] & bit != 0) revert AlreadyMinted();
