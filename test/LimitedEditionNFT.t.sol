...
contract LimitedEditionNFTTest is Test {
  ...
  function setUp() public {
    ...
    tokenNFT = new LimitedEditionNFT(BASE_PRICE, DISCOUNT_PRICE, merkleRoot);
  ...
  function testDefaultRoyalty() public {
    uint256 nftId = 1000;
    tokenNFT.mint{ value: BASE_PRICE }(alice);

    (address receiver, uint256 royalty) = tokenNFT.royaltyInfo(nftId, BASE_PRICE);
    assertEq(receiver, address(this), 'Receiver should be tokenNFT');
  ...
  function testMintNFTWithMerkleProof() public {
    ...
    tokenNFT.mint{ value: DISCOUNT_PRICE }(alice, proofEmpty);

    bytes32[] memory proofAlice = merkle.getProof(leaves, 0);
    tokenNFT.mint{ value: DISCOUNT_PRICE }(alice, proofAlice);
    ...
    tokenNFT.mint{ value: DISCOUNT_PRICE - 1 }(bob, proofBob);
    tokenNFT.mint{ value: DISCOUNT_PRICE }(bob, proofBob);
    ...
    tokenNFT.mint{ value: DISCOUNT_PRICE }(alice, proofBob);

    vm.expectRevert();
    tokenNFT.mint{ value: DISCOUNT_PRICE }(zeroAddress, proofBob);
  }

  function testSupportsInterface() public {
    assertTrue(tokenNFT.supportsInterface(type(IERC721).interfaceId));
    assertTrue(tokenNFT.supportsInterface(type(IERC2981).interfaceId));
    assertTrue(tokenNFT.supportsInterface(type(IERC165).interfaceId));
    assertFalse(tokenNFT.supportsInterface(type(IERC1967).interfaceId));
  }
  ...
  function testAliceSuccessMintNFT() public {
    ...
    uint256 remainingSupply = tokenNFT.remainingSupply();
  ...
  function testAliceAndBobSuccessMintNFT() public {
    ...
    tokenNFT.mint{ value: BASE_PRICE }(bob);
    tokenNFT.mint{ value: BASE_PRICE }(bob);
    uint256 bobNftBalanceAfter = tokenNFT.balanceOf(bob);
  ...
  function testInvalidAddressRevert() public {
    ...
    tokenNFT.mint{ value: BASE_PRICE }(address(0));
  }
  ...
  function testWithdraw() public {
    ...
    uint256 initNFTBal = address(tokenNFT).balance;
    ...
    uint256 finalNFTBal = address(tokenNFT).balance;
    ...
    tokenNFT.withdraw(payableAddr, BASE_PRICE);

    assertEq(address(tokenNFT).balance, finalNFTBal - BASE_PRICE, 'Balance should be less than final');
    ...
    tokenNFT.withdraw(payableAddr, finalNFTBal + 1);
  }
  ...
  function testValidateMint() public {
    ...
    vm.startPrank(alice);
    vm.expectRevert(abi.encodeWithSignature('InsufficientFunds()'));
    tokenNFT.mint{ value: 199 gwei }(alice);
    vm.stopPrank();
  ...
  function testOnERC721Received() public {
    ...
    manager.onERC721Received(address(tokenNFT), alice, nftId, '');
    vm.stopPrank();

    vm.startPrank(address(tokenNFT));
    vm.expectEmit(address(manager));
    emit Staked(alice, nftId);
    bytes4 data = manager.onERC721Received(address(tokenNFT), alice, nftId, '');
